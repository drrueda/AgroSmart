...
if (path.exists(rinde))&(path.exists(resumen)):
    ...
    Perd_capa = df_resumen.groupby('Capa').agg(Perdidas=('Perd_kg_h','sum'),Poc_=('Porc_Perd','sum')).reset_index()\
                                .sort_values(by='Perdidas',ascending = False)
    ...
    if capa == 'Resumen':
        ...
        fig.add_trace(go.Table(header=dict(values=rinde_g.columns.to_list(),font=dict(size=12),line = dict(color='rgb(50, 50, 50)'),
    	...
    	fig.update_annotations(font_size=24)
    	fig.update_layout(height=600, width=900, title_text='Capa: '+capa,showlegend=False,title_font_size=20)
    	st.write(fig)
    	#st.write(rinde_g)
else: 
